...
export function save(client, obj, table, attrs, ver, buildParam) {
...
export function saveBatchWithClient(client, objs, table, attrs, ver, buildParam) {
  const s = buildToSaveBatch(objs, table, attrs, ver, buildParam);
  if (s.length === 0) {
    return Promise.resolve(0);
  }
...
export class PostgreSQLStreamWriter {
  constructor(con, table, attributes, size, toDB, buildParam) {
    this.table = table;
    this.attributes = attributes;
    this.list = [];
    this.size = 0;
...
export class StringService {
  constructor(table, field, queryData, execute) {
    this.table = table;
    this.field = field;
