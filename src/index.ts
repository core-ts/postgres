...
export class StringService {
  ...
  load(keyword: string, max?: number): Promise<string[]> {
    ...
    const k = keyword + "%"
    return this.query(`select ${this.field} from ${this.table} where ${this.field} ilike $1 order by ${this.field} limit ${m}`, [k]).then((res) =>
      res.map((i) => (i as any)[this.field]),
    )
  }
  save(values: string[]): Promise<number> {
    if (!values || values.length === 0) {
...
// tslint:disable-next-line:array-type
export function useUrlQuery<ID>(
  queryF: <T>(sql: string, args?: any[]) => Promise<T[]>,
  table: string,
  url?: string,
  id?: string,
  name?: string,
  displayName?: string,
): (ids: ID[]) => Promise<URL<ID>[]> {
